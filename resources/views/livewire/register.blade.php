<div class="auth-page">
    <div class="container page">
        <div class="row">
            <div class="col-md-6 offset-md-3 col-xs-12">
                <h1 class="text-xs-center">Sign up</h1>
                <p class="text-xs-center">
                    <a wire:navigate href="/login">Have an account?</a>
                </p>

                <ul class="error-messages">
                    @error('username')
                    <li>{{ $message }}</li>
                    @enderror

                    @error('email')
                    <li>{{ $message }}</li>
                    @enderror

                    @error('password')
                    <li>{{ $message }}</li>
                    @enderror
                </ul>

                @if($authenticated)
                    <h1>asdf</h1>
                @endif

                <form wire:submit="save">
                    <fieldset class="form-group">
                        <input wire:model.live="username" class="form-control form-control-lg" type="text" required
                               placeholder="Username"/>
                    </fieldset>
                    <fieldset class="form-group">
                        <input wire:model="email" class="form-control form-control-lg" type="email" required
                               placeholder="Email"/>
                    </fieldset>
                    <fieldset class="form-group">
                        <input wire:model="password" class="form-control form-control-lg" type="password" required
                               placeholder="Password"/>
                    </fieldset>
                    <button class="btn btn-lg btn-primary pull-xs-right" type="submit">Sign up</button>
                </form>
            </div>
        </div>
    </div>
</div>
